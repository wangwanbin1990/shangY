...
public class CacheService {
	...
	public static void main(String[] args) {
		CacheService.getInstance().remove("867fe9506d704b74a7c75b5345db58a1");
		CacheService.getInstance().remove("string");
	}
			...
			try {
				...
				if (serverNum > 0) {
					...
					for (int i = 1; i <= serverNum; i++) {
						...
						jedisShardInfo = new JedisShardInfo(redisIp, redisPort, getConfig().getInt(JEDIS_TIMEOUT));
	...
	private ShardedJedis getCache() {
		...
		do {
			...
			count++;
		} while (jedis == null && count < getConfig().getInt(JEDIS_RETRYNUM));
		return jedis;
	}
	...
	public String getString(String key) {
		...
		try {
			...
			if (jedis.exists(key)) {
				value = jedis.get(key);
				value = org.apache.commons.lang.StringUtils.isNotBlank(value) && !"null".equalsIgnoreCase(value) ? value
						: null;
			}
		} catch (Exception e) {
			isBroken = true;
	 ...
	 * @param mapKey
	 * @param mapValue
	 *            用兩個redis的key-value保存結果 第一個：jedisKey：set<setKey> 第二個：setKey:Map
	 *            <mapKey,mapValue>
	 */
	 ...
	 * 
	 * @param jedisKey
	 *            取用兩個redis的key-value保存的結果 第一個：jedisKey：set
	 *            <setKey> 第二個：setKey:Map<mapKey,mapValue>
	 */
				...
				for (Iterator<String> it = set.iterator(); it.hasNext();) {
					...
					if (map != null && map.size() > 0) {
						for (Entry<String, String> entry : map.entrySet()) {
							System.out.println("jedisKey:" + jedisKey + ",jedisMapKey:" + datestring + ",mapKey:"
									+ entry.getKey() + ",mapValue:" + entry.getValue());
						}
					}
				}
			}
